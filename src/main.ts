...
const catalog = new ProductList(events);
...
const modalSuccessTemplate = cloneTemplate<HTMLElement>('#success');
const modalSuccess = new ModalSuccess(events, modalSuccessTemplate);
...
events.on('catalog:updated', (data: { products: IProduct[] }) => {
  const catalogElements = data.products.map(item => {
    ...
    const productCard = new ProductCard(events, cardElement);
...
events.on('catalog:detailedUpdated', (data: { detailedProduct: IProduct }) => {
  const productPreviewTemplate = cloneTemplate<HTMLElement>('#card-preview');
  const productPreview = new ProductPreview(events, productPreviewTemplate);
  const cardContent = productPreview.render({
...
events.on('cart:updated', (data: { items: IProduct[], total: number, count: number }) => {
  ...
  const basketItems = data.items.map((item, index) => {
    ...
    const basketItem = new BasketItem(events, cardElement);
    return basketItem.render({ id: item.id, title: item.title, price: item.price, index: index + 1 });
  });
...
events.on('order:submit', () => {
  const buyerInfo = buyer.getInfo();
  const validation = buyer.validate();
  const errors = [validation.email, validation.phone].filter((e): e is string => Boolean(e));
  modal.render({ content: contactsForm.render({ email: buyerInfo.email, phone: buyerInfo.phone, valid: errors.length === 0, errors }), isOpen: true });
});
...
events.on('contacts:submit', () => {
  ...
  shopApi.submitOrder(orderData)
    .then(() => {
      modalSuccess.total = orderData.total;
      modal.render({ content: modalSuccess.render(), isOpen: true });
      cart.clearCart();
      buyer.clearInfo();
    })
